...
// chargement du fichier data.js
var WIDTH = 1200;
var HEIGHT = 500;
var MARGIN = 20; // espace entre les batons

var MARGIN_TOP = HEIGHT / 10;
var MARGIN_BOTTOM = HEIGHT / 7; // on fait de la place pour les noms
...
var BAR_WIDTH = GRAPH_WIDTH / _data.default.length; // largeur des batons
// Echelle hauteur
// Conversion du range des données en range de pixel

var yScale = d3.scaleLinear().domain([0, d3.max(_data.default, function (d) {
  return d.nb;
})]).range([GRAPH_HEIGHT - MARGIN_TOP, 0]);
var bars = svg.append('g').attr('transform', "translate(".concat(MARGIN_LEFT, ", ").concat(MARGIN_TOP, ")"));
bars.selectAll('rect').data(_data.default).enter().append('rect').attr('x', function (d, i) {
  return i * BAR_WIDTH;
...
.attr('height', function (d) {
  return GRAPH_HEIGHT - MARGIN_TOP - yScale(d.nb);
}) // hauteur
.attr('fill', '#ECC35A').attr('rx', 3);
var dates = svg.append('g').attr('transform', "translate(".concat(MARGIN_LEFT - 17, ", 20)"));
dates.selectAll('text').data(_data.default).enter().append('text').attr('x', function (d, i) {
  return i * BAR_WIDTH + BAR_WIDTH / 2;
}).attr('y', HEIGHT - MARGIN_BOTTOM + 20).attr('text-anchor', 'middle').attr('font-family', 'sans-serif').attr('font-size', 13).style('fill', 'white').attr('transform', function (d, i) {
  var x = i * BAR_WIDTH + BAR_WIDTH / 2;
  var y = HEIGHT - MARGIN_BOTTOM + 20;
  return "rotate(-65, ".concat(x, ", ").concat(y, ")");
}).text(function (d) {
  return d.date;
});
var axisY = d3.axisLeft().scale(yScale).tickFormat(function (d) {
  return "".concat(d / 1, " pas");
}).ticks(5);
svg.append('g').attr('transform', "translate(".concat(MARGIN_LEFT - 20, ", ").concat(MARGIN_TOP, ")")).call(axisY).attr('font-size', 13).attr('color', 'white');
},{"d3":"../../node_modules/d3/index.js","./data":"data.js"}],"../../node_modules/parcel-bundler/src/builtins/hmr-runtime.js":[function(require,module,exports) {
